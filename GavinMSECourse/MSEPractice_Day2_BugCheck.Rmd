...
dynamics <- function(pars,C,yrs, m) {
  ...
  for (y in 2:nyr) {
    B[y] <- PT(B[y-1],C[y-1],K,r, m= m) # Why not? B[y] <- rnorm(1, PT(B[y-1],C[y-1],K,r, m), sd(harvest))
  }
...
#       biomass.i[y+1] <- PT(biomass.i[y],catch.i[y],K.i,r.i,m=m)
#       
...
for (i in 1:niter) {
  ...
  biomass.iter <- rbind(biomass.iter,
                        data.frame(year = seq(min(data.years),
                                              max(data.years)+1),
                                   biomass = dynamics(pars.iter[i,], harvest, m = 1.2),
                                   iter = i))
}
biomass.iter
