...
ecov_om <- list(
  label = "bt_temp",
  mean = as.matrix(c(env.dat_me$bt_temp, rep(NA, 4))), # Future values will be filled
  logsigma = "est_1",
  year = 1972:2025,
...
projection_om <- data.frame(year_proj = c(2022:2025))
...
ecov_om_P <- ecov_om
ecov_om_P$mean[51:54,] <- projection_om$Temp_proj
...
finalize_om_for_simulation <- function(input, scenario_type, Ecov_re, ecov_lmean) {
  ...
  if (scenario_type == "P") {
    ...
    input$par$Ecov_re[52:54,] <- input$data$Ecov_obs[52:54,] - ecov_lmean
...
for(i in 0:4){
  if (i %% 3 == 0){
    HistAvg[(i+1):(i+3)] <- mean(ecov_me$mean[(1:k)], na.rm = TRUE)
    RecWind[(i+1):(i+3)] <- mean(ecov_me$mean[((k-4):k)], na.rm = TRUE)
    om_RecentTrend <- summary(lm(ecov_me$mean[((k-4):k)] ~ ecov_me$year[((k-4):k)]))
    RecTrend$Temp[(i+1):(i+3)] <- om_RecentTrend[["coefficients"]][1] + (om_RecentTrend[["coefficients"]][2] * ecov_me$year[k])
  }
  k <- k + 1
}

ecov_HistAvg <- ecov_me
ecov_HistAvg$mean[51:54,] <- HistAvg[1:4]
# ecov_HistAvg$mean[51:81,] <- 0

ecov_RecWind <- ecov_me
ecov_RecWind$mean[51:54,] <- RecWind[1:4]

ecov_RecTrend <- ecov_me
ecov_RecTrend$mean[51:54,] <- RecTrend$Temp[1:4]
...
foreach (i = c(1:10)) %dopar% {
  ...
  saveRDS(mod, file.path(sprintf("C:/Users/swulfing/OneDrive - University of Massachusetts Dartmouth/Desktop/MSE_results/Mod.3yr_1_%03d.RDS", i)))
  }, error=function(e){})
}
...
foreach (i = 1:10) %dopar% {
  ...
  saveRDS(mod, file.path(sprintf("C:/Users/swulfing/OneDrive - University of Massachusetts Dartmouth/Desktop/MSE_results/Mod.3yr_2_%03d.RDS", i)))
    }, error=function(e){})
}
...
foreach (i = 1:10) %dopar% {
  ...
  saveRDS(mod, file.path(sprintf("C:/Users/swulfing/OneDrive - University of Massachusetts Dartmouth/Desktop/MSE_results/Mod.3yr_3_%03d.RDS", i)))
    }, error=function(e){})
}
...
foreach (i = 1:10) %dopar% {
  ...
  saveRDS(mod, file.path(sprintf("C:/Users/swulfing/OneDrive - University of Massachusetts Dartmouth/Desktop/MSE_results/Mod.3yr_4_%03d.RDS", i)))
    }, error=function(e){})
}
...
foreach (i = 1:10) %dopar% {
  ...
  saveRDS(mod, file.path(sprintf("C:/Users/swulfing/OneDrive - University of Massachusetts Dartmouth/Desktop/MSE_results/Mod.3yr_5_%03d.RDS", i)))
    }, error=function(e){})
}
...
mods <- lapply(nsim, function(r) {
  ...
  mod_list <- lapply(model_nums, function(m) {
    file_path <- file.path(sprintf("C:/Users/swulfing/OneDrive - University of Massachusetts Dartmouth/Desktop/MSE_results/Mod.3yr_%d_%03d.RDS", m, r))
    readRDS(file_path)
  })
...
saveRDS(mods, file = "C:/Users/swulfing/OneDrive - University of Massachusetts Dartmouth/Desktop/MSE_results/MSEmods_3yr.rds")
