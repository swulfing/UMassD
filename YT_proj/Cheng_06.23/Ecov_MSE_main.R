...
#####################################################################
# Bug here, Year needs to be 1971 becuase of the "1-year" lag effect!
#####################################################################
...
# Here I try not estimate sigma of ecov time series, so 'restrain' some noise.
ecov_me <- list(
  label = "bt_temp",
  mean = as.matrix(env.dat_me$bt_temp),
  logsigma = as.matrix(rep(log(0.2),length(env.dat_me$bt_temp))), # Don't estimate first
...
om <- fit_wham(input_Ecov, do.fit = T, do.brps = T, MakeADFun.silent = FALSE, do.retro = FALSE, do.osa = FALSE)
...
# You should be able to compare these 3 models 
# compare_wham_models(mods = list(mod1 = self_fit, mod2 = cross_fit, mod3 = cross_fit2))
# Comparing cross_fit with other models is not possible because cross_fit does not have ecov time series, so in short, data used for model fitting is not the same! 
compare_wham_models(mods = list(mod1 = self_fit, mod3 = cross_fit2)) # You can compare these 2

# You can see mod3 is the winner in AIC and retro!

##################################################
######### Try with sigma on ecov estimated #######
##################################################
...
om2 <- fit_wham(input_Ecov, do.fit = T, do.brps = T, MakeADFun.silent = FALSE, do.retro = FALSE, do.osa = FALSE)

plot_wham_output(om2, out.type = "html")

om2$sdrep

# You can see the estimated pars. associated with ecov using om2$sdrep
# Ecov_process_pars   7.38116469 0.21150514
# Ecov_process_pars  -1.02371643 0.38789614
# Ecov_process_pars   1.99234965 0.79601766
# Ecov_obs_logsigma  -0.86069102 0.24534655


# Self- and Cross-Test
set.seed(12345)
self_fit <- sim_fn(om2, self.fit = TRUE)
set.seed(12345)
cross_fit2 <- sim_fn3(om2, cross.fit = TRUE)

compare_wham_models(mods = list(mod1 = self_fit, mod3 = cross_fit2)) # You can compare these 2 models 

# You can clearly see the one without ecov performs even better than the 'correct' model!
# WHY? Perhaps because you used a density-dependent recruitment model (B-H) and seems wham prefers to impose 
# the noise from the ecov (ecov effect) to the density-dependent mechanism. This is not uncommon, if you look at 
# the ability of B-H in shaping the recruitment fluctuation.

# Suggestions: 1) continue working on B-H model, but tinkering with logsigma (try higher value and lower values)
# 2) try to use rec model = 2 (random around the mean!)
