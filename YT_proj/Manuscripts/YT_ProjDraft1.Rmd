...
```{r recruitmentDevs, fig.cap = 'Percent difference of the recruiement deviation per assessment year in each EM compared to the OM.', echo = FALSE, warning = FALSE, message = FALSE}
...
MSYOverfishedStatus <- BRP_data %>%
  filter(Year >= 2024) %>%
  group_by(EM_name, Year) %>%
  summarize(OM_Overfishing = log10(mean(MSYOverfishing_OM, na.rm = TRUE)),
            EM_Overfishing = log10(mean(MSYOverfishing_EM, na.rm = TRUE)),
            SD_Overfishing = log10(sd(MSYOverfishing_EM, na.rm = TRUE)),
