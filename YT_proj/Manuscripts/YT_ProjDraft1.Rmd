...
```{r recruitmentDevs, fig.cap = 'Percent of recruitment deviation EXPLAINED BY THE ECOV IN EACH  MODEL. MAKE SURE YOU CHANGE YOUR RESULTS PARAGRAPHS TO SAY THIS', echo = FALSE, warning = FALSE, message = FALSE}
...
#   filter(Year >= 2022) %>%
#   group_by(Year, EM_name) %>%
#   summarise(ResidRecruit = mean(EM_NAAsigma_youngest, na.rm = TRUE))
# 
# p <- list()
# 
# for(i in 1:length(unique(data_recruit$EM_name))){
#   datause <- data_recruit %>%
#     filter(EM_name == unique(data_recruit$EM_name)[i])
#   
#   p[[i]] <- acf(datause$ResidRecruit)
# }
# 
# ggpubr::ggarrange(p[[1]], p[[2]], p[[3]], p[[4]], p[[5]], p[[6]], p[[7]], p[[8]], nrow = 4, common.legend = TRUE, legend = 'right')
