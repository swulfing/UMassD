...
## New Calc
# Projection
cohortsP$Recruit_sigma <- exp(cohortsP$Recruit_sigma)
cohortsP$Ecov_sigma <- exp(cohortsP$Ecov_sigma)

cohorts_PROJ <- cohortsP %>%
  mutate(contribution = (Ecov_sigma^2)/((Ecov_sigma^2) + (Recruit_sigma^2)))

# NAA
cohortsN$Recruit_sigma <- exp(cohortsN$Recruit_sigma)
cohortsN$Ecov_sigma <- exp(cohortsN$Ecov_sigma)

cohorts_NAARE <- cohortsN %>%
  mutate(contribution = (Ecov_sigma^2)/((Ecov_sigma^2) + (Recruit_sigma^2)))

# 5 YR
cohorts5$Recruit_sigma <- exp(cohorts5$Recruit_sigma)
cohorts5$Ecov_sigma <- exp(cohorts5$Ecov_sigma)

cohorts_5YR <- cohorts5 %>%
  mutate(contribution = (Ecov_sigma^2)/((Ecov_sigma^2) + (Recruit_sigma^2)))

## Old Calc:
## MSEmods_50yr[[1]]$Mod1$om$par[grep("Ecov_re", names(MSEmods_50yr[[1]]$Mod1$om$par))]
# 
