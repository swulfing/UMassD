FIXEDplot_ssb_variation <- function (mods, is.nsim, main.dir, sub.dir, var = "SSB", width = 10, 
          ...
          base.model = NULL) 
{
  ...
  if (!is.nsim) {
    ...
    res <- lapply(seq_along(mods), function(i) {
      ...
      colnames(tmp)[1:n_regions] <- paste0(var, "_Region", 
                                           seq_len(n_regions))
  ...
  res <- res %>% filter(Label == 'SSB_Global')
