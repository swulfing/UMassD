...
q = c(2.134e-4,2.2267e-4,2.727e-4) # FROM fitted_om$rep$q
...
sel3=list(
  ...
  re = c("ar1_y","none","none","none"), #re = c("ar1_y","none","none","none"), use ar1 to est sel but later fixing all 
  initial_pars=list(
    c(0.016,0.242,0.743,1,1,1), # Commercial fleet (used closest to 48)
...
# I used the sigma vals from my fitted_om, but you can use whatever you want. FROM PARAM TABLE FITTED OM PLOTTED OUTPUT
...
# IF you choose to use B-H recruitment model, then you need to specify alpha and beta for B-H otherwise the default value will be used! FROM PARAM TABLE OF FITTED OM PLOTTED OUTPUT
...
```{r}
# ### PREPPING ECOV FOR MSE ### 
# 
# ecov_none$mean <- rep(NA, 60)
# ecov_none$year <- 1973:2032 #1:53
# ecov_none$use_obs <- matrix(0, ncol = 1, nrow = (49+11))
# 
# #############################
...
saveRDS(mod, file = "MSE_NOECOV.rds")
